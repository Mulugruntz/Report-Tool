...
class EquityChart(pg.PlotWidget):
    ...
    def __init__(self, *args, **kwargs):
        ...
        date_axis = classCustomGraphicsItems.DateAxis({},
                                                      orientation="bottom")
    ...
    def plot_curve(self, ec_color, ec_size, ec_style,  *args, **kwargs):
        ...
        style_dict = {"Solid": 1,
                      ...
                      "Dot": 3,
                      "Dash Dot": 4,
                      "Dash Dot Dot": 5}    # dict with avalaible QtStyle

        qt_ec_style = QtCore.Qt.PenStyle(style_dict[ec_style])

        equity_pen = pg.mkPen(color=QtGui.QColor(ec_color),
                              width=ec_size,
                              style=qt_ec_style,
                              )
    ...
    def plot_scatter(self, color, symbol, size, *args, **kwargs):
        ...
        scatter_plot = pg.ScatterPlotItem(symbol=symbol,
                                          size=size,
                                          pen=pg.mkPen(None),
                                          brush=scatter_brush
                                          )    # init plot
                ...
                if x_pos <= x_range[0]:    # line pos is smaller than x min
                    ...
                    try:
                        ...
                        if x_max >= max(data_plotted[0]):
                            ...
                            data_in_range = data_plotted[1][x_min:max(data_plotted[0])+1]
    ...
    def add_text_item(self, curve, comment, deal_id, graph):
        ...
        if deal_id not in comments_items.keys() and text != "":
            ...
            arrow.setStyle(tipAngle=30,
                           baseAngle=20,
                           headLen=head_len,
                           tailLen=tail_len,
                           tailWidth=3,
                           pen=pg.mkPen(60,60,60, width=1),
                           brush=pg.mkBrush(60,60,60))    # set fixed style
            ...
            text_item = pg.TextItem(anchor=anchor,
    ...
    def update_text_item(self, curve, *args, **kwargs):
        ...
        try:    # when user edits a comment
            ...
            html_text ='<div style="text-align: center">\
                        <span style="color: rgb(0, 0, 0);">'\
                         + html_text + '</span></div>'    # set style for text
        ...
        for count, deal_id in enumerate(comments_items.keys()):
            ...
            text_item.anchor = QtCore.QPointF(anchor[0], anchor[1])    # update text anchor
            ...
            text_scene_coord = self.plotItem.vb\
                                   .mapViewToScene(QtCore.QPointF(point_x, point_y))
            ...
            text_view_coord = self.plotItem.vb\
                                  .mapSceneToView(QtCore.QPointF(text_pos_x,
                                                                 text_pos_y))
